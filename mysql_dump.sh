...
backup() {
  ...
  FULL_BACKUP_FILENAME="$4_$(date +"%m-%d-%Y")-$(date +%s)_FULL.sql.gz"
  MINIMIZED_BACKUP_FILENAME="$4_$(date +"%m-%d-%Y")-$(date +%s)_MINIMIZED.sql.gz"
  # Mysqldump notes:
  ...
  for table_name in "${table_names[@]}"; do
    mysqldump --opt --quick --single-transaction --user=$1 --password=$2 $3 $table_name | gzip >> "${FULL_BACKUP_FILENAME}"
  done;
}
